...
def ipv4_to_value(ipv4_addr):
    ...
    """
    ipn = [int(n) for n in ipv4_addr.split(".")]
    ipb = 0
...
def find_router_for_ip(routers: dict, ip: str):
    ...
    for router_ip, ip_info in routers.items():
        slash = ip_info["netmask"]
...
def my_tests():
    ...
    ip = "1.2.3.5"
    if find_router_for_ip(routers, ip) == "1.2.3.1":
        print("PASS")
    else:
        print("FAIL")
    ...
    ip = "1.2.5.6"
    if find_router_for_ip(routers, ip) == None:
        print("PASS")
    else:
        print("FAIL")
"""
